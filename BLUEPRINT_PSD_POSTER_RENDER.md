# BLUEPRINT_PSD_POSTER_RENDER.md - PSD Poster Rendering Pipeline

**STATUS:** BLUEPRINT_ACTIVE
**PURPOSE:** Preserve the full PSD poster rendering script for recreation, reference, and fast redeploy inside the Management App.

## Overview
This blueprint stores the exact, full script used to render PSD posters with custom main text and subtext.
...
def apply_gradient_overlay(base: Image.Image):
    ...
    if grad.size != base.size:
        grad = grad.resize(base.size, Image.LANCZOS)
...
def choose_red_word_indices(words: List[str], max_red: int = 2, red_words: Optional[Set[str]] = None) -> set:
    """Pick word indices to render in red."""
    if not words:
        return set()
    if red_words:
        red_indices = set()
...
def render(
    ...
    # Re-apply logo over background if present
    ...
    if subline and subtext_layer is not None:
        norm_subline = subline
        if subline and SUBTEXT_FORCE_SENTENCE_CASE:
            norm_subline = subline[:1].upper() + subline[1:].lower()
        draw_subtext_exact(
            base,
            subtext_layer,
            norm_subline,
